...
async def fetch_github_content(github_link: str, max_file_size: int) -> dict:
    try:
        ...
        os.environ["GIT_HTTP_LOW_SPEED_TIME"] = "999999"

        # Increase timeout for large repositories (5 minutes for git operations)
        summary, tree, content = await asyncio.wait_for(
            ingest_async(source=github_link, max_file_size=max_file_size),
        ...
        logging.error(
            f"Error fetching content for {github_link}: {str(e)}", exc_info=True
        )
        raise HTTPException(
            status_code=500, detail=f"Failed to ingest repository: {str(e)}"
        )
